...
_video_page_ref: dict = {"page": None, "config": None, "save_ui": None}


def save_all_video_configs() -> None:
    """供主界面关闭时调用：保存同步页中的配置。"""
    page = _video_page_ref.get("page")
    if page is not None and page.winfo_exists():
        try:
            save_ui = _video_page_ref.get("save_ui")
            if save_ui is not None:
                save_ui()
            config = _video_page_ref.get("config")
...
def create_video_sync_page(parent: tk.Widget, root: tk.Misc, on_resize=None) -> None:
    """在 parent 中构建 视频同步页面。root 用于定时器/对话框父窗口。"""
    config = VideoSyncConfig.load()
    ctrl = VideoSyncController(config)
    state = {"busy": False}
    ...
    # ---- 页面主体 ----
    frame_sync = ttk.LabelFrame(parent, text=" 视频同步（仅 USB 连接） ", padding=6)
    ...
    def sync_save_ui() -> None:
        ...
        config.device_folder = ent_sync_device.get().strip() or "/sdcard/Video"
    ...
    def sync_run() -> None:
        ...
        if not config.local_folder or not os.path.isdir(config.local_folder):
            messagebox.showwarning("提示", "请先选择有效的本地视频目录", parent=root)
            return
        ctrl.busy = False
        state["busy"] = True
        sync_set_busy(True)
    ...
    _video_page_ref["page"] = parent
    _video_page_ref["config"] = config
    _video_page_ref["save_ui"] = sync_save_ui