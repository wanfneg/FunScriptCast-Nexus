...
# -*- coding: utf-8 -*-
"""
视频文件夹同步模块（逻辑与 Funscript 脚本同步一致）
=============================================
在 VR-DLNA 中提供 PC 视频目录 -> Android 设备的增量同步能力。

功能与 Funscript 脚本同步对齐：
  * 扫描本地视频文件
  * 增量（设备缺失）或强制全量同步
  * 直接 adb push 到设备（跳过设备端 unzip，速度更快）
  * push 后大小校验（防传输损坏）
  * 可选删除设备上多余 video
  * 数量校验
...
CONFIG_FILE = _sync_data_dir() / "vr_dlna_video_config.json"
...
# 视频文件扩展名（与 DLNA 支持的常用视频格式一致）
VIDEO_EXTS = (
    ".mp4", ".mkv", ".mov", ".avi", ".webm", ".ts", ".m2ts", ".mts",
    ".wmv", ".flv", ".mpg", ".mpeg", ".3gp", ".ogv", ".vob",
)
...
class VideoSyncConfig:
    local_folder: str = ""
    device_folder: str = "/sdcard/Video"
    adb_path: str = ""
    force_full: bool = False
    delete_extra: bool = False

    @staticmethod
    def load() -> "VideoSyncConfig":
        try:
...
class VideoSyncEngine:
    ...
    def __init__(self, local_folder: str, device_folder: str, serial: str, adb: AdbClient):
        ...
        self.device_folder = (device_folder or "/sdcard/Video").rstrip("/") or "/"
        self.serial = serial
        self.adb = adb
    ...
    def scan_local(self) -> dict[str, str]:
        ...
        → 删除判定会把设备上对应的媒体文件全部当成"多余"删掉。改成 os.walk + onerror
    ...
    def scan_device(self) -> set[str]:
        """设备上的视频相对路径集合（实际路径字符串）。"""
        result: set[str] = set()
        # -iname 而非 -name：本地判据是 suffix.lower()（大小写不敏感），设备端若用
        # 大小写敏感的 -name，则 .MP4 这类大写扩展名永远匹配不上，每轮都被判成
        # "设备上没有"而重复推送。只是白推，不会误删，但永不收敛。
        expr = " \\( " + " -o ".join(f"-iname '*{ext}'" for ext in VIDEO_EXTS) + " \\)"
        cmd = f"find {_shq(self.device_folder)} {expr} -type f 2>/dev/null"
    ...
    def run(self) -> tuple[int, int, int]:
        ...
        # （"/sdcard/Video/.." 规范化后正好是 /sdcard），护栏等于没有。
        ...
        self._emit(f"本地视频共 {len(local)} 个")
        ...
        pushed = 0
        if to_send:
            # 视频文件本身通常已是高压缩率格式，zip 几乎无收益；
            # 直接 adb push 可彻底跳过设备端 unzip，速度明显更快。
            self._emit("直接推送视频文件到设备（跳过设备端解压）...")
            total = len(to_send)
            made_dirs: set[str] = set()
            for idx, rel in enumerate(to_send, 1):
                src = os.path.join(self.local_folder, rel.replace("/", os.sep))
                if not os.path.isfile(src):
                    self._emit(f"跳过已消失文件：{rel}")
                    continue
                remote = f"{self.device_folder}/{rel}"
                remote_dir = remote.rsplit("/", 1)[0]
                if remote_dir not in made_dirs:
                    self.adb.shell(self.serial, f"mkdir -p {_shq(remote_dir)}")
                    made_dirs.add(remote_dir)
                self.adb.push(self.serial, src, remote)
                pushed += 1
                if idx % 10 == 0 or idx == total:
                    self._emit(f"已推送 {pushed}/{total}")
        ...
        if self.delete_extra and (not local or self.last_scan_incomplete):
            ...
            # 导致本地看起来比设备少，紧接着就是 rm -f 把设备媒体库清空。这一轮不删
...
class VideoSyncController:
    ...
    def __init__(self, config: VideoSyncConfig):
        self.config = config
        self.serial = ""
        self.busy = False
        self.on_log: Optional[Callable[[str], None]] = None
    ...
    def run_sync(self) -> Optional[tuple[int, int, int]]:
        ...
        if not self.config.local_folder or not os.path.isdir(self.config.local_folder):
            self._emit("请先选择有效的本地视频目录")
            raise InvalidOperationException("请先选择有效的本地视频目录")
        if not self.serial:
