...
## R73 应用内更新检测 + 就地升级（4839a0d，v1.0.35）
...
## R74 移除 adb 路径自定义入口（b416960，v1.0.36）
...
## R75 UI 布局梳理（4fc8f6a，纯 ui\ 热修，无版本号递增）

用户点名：同步页删 adb 字段后留下稀疏空洞；设置页卡片大小不一、底边不平齐；要求全 UI 布局审查。

实现（全部 CSS/HTML 减法，未动任何逻辑）：
1. **`.grid` 去掉 `align-items:start`**：同行卡片恢复缺省 stretch 拉伸到等高——设置页 启动/外观、更新/AI字幕 两行底边平齐；同步页 脚本/视频卡等高。
2. **同步页 设备连接卡重排**：选择设备改全宽（原 g2 只剩一个半宽字段）；两个开关（全量同步/删除多余文件）改为左右各半、各自"名称左+开关右"，不再扯到卡片两角。
3. **AI 字幕页**：翻译后端 移出两列网格改全宽，与下方 日语/英语翻译模型 对齐（原第三项落单成半宽孤行）。
4. 全页浏览器截图审查：仪表盘/DLNA 无问题未动；每页截图核验过修改效果。

⚠️ 教训重申：删字段不只是删输入框——它所在的栅格布局会留下空洞，**删 UI 必须连排版一起重排 + 截图回看**。ui\ 纯样式/结构改动可热修三处（仓库/dist-app/D 盘）刷新即生效，无需重编 exe。

---

## R76 应用内更新安装链路重做 + 下载前台进度（本条与提交同步进行，v1.0.38）

用户实测 1.0.36 的"立即安装"：先弹黑色命令行窗口，再弹报错，安装失败。根因两个：
1. 交接进程用 `cmd + ping -n 4` 做延迟，DETACHED_PROCESS 下 ping 自己弹可见控制台窗口；
2. 固定等 3 秒赌宿主退出速度——Setup 撞上还没退干净的宿主（文件占用），/SUPPRESSMSGBOXES 下默认 Abort 并回滚（Setup 日志实锤："无法自动关闭所有应用程序…Defaulting to Abort"）。另外宿主下线判定用端口也不对：端口关了 ≠ 进程退出、文件锁释放。
...
## R80 审查报告第四批修复：F02 / F10 / F18 / F11（并发正确性三件 + 空闲计时口径）

### F02 [中] 设置缓存读写无锁 —— 属实，已修（含一处报告没提到的第二半）

报告描述的错配确实存在：命中判定与取值在锁外，key/data 又是**两条独立语句**写。实测复现
（新增竞态测试，把 `switchinterval` 调到 1e-6、4 个读线程 + 一个写线程）：
旧代码 3/3 次都抓到 `key 与文件当前 stat 相符、data 却是旧内容` 的错配缓存。

修法：①命中判定与取值在同一把锁内、key/data 作为**一对**读取；②写回也成对；
③⚠ **报告没提到的第二半**：我按报告只做①②后，测试**仍然失败**（3 次里 2 次）——
根因是缓存键 `(mtime_ns, size)` 会**碰撞**：两次"大小相同"的保存若落在同一 100ns 刻度上，
键就一样 ⇒ 命中旧内容。所以又给 `save_settings` 加了**写透缓存**（保存方本来就知道新内容
与新 stat，写完成对回填）。改完后连跑 5 次全过，旧代码 3/3 失败。
这条很值得记：**照报告改完不等于修好，必须让测试说话**。

### F10 [中] 在飞计数与时钟顺序 —— 属实，已修

`/transcribe` 先 `await read_capped_body` 才 `_INFLIGHT += 1`（首个请求上传期间计数为 0，
空闲回收临界点会把服务 os._exit）；finally 里先减计数、后刷时钟（超长请求在间隙里被判空闲）。
两个入口口径还不一致（stream 本来就是先自增）。

修法：进 handler 第一件事自增（超大/超长提前返回也走 finally 归还）；
finally 改为**先刷时钟再减计数**；两个入口统一。

### F18 [低] 空闲计时用墙钟 —— 属实，已修

判定改用 `time.monotonic()`。**但对外展示仍是墙钟**：`ui/app.js:354` 按
`Date.now()/1000 - h.last_req_ts` 算"多久以前"，改成单调钟会让界面显示错。所以拆成两个
变量（`_LAST_REQ_MONO` / `_LAST_REQ_WALL`），在 `_touch_request_clock()` 里一起打点，
并新增 `idle_sec` 字段给界面用。

### F11 [中] 会话状态进程级单份、多设备串台 —— 属实，已按报告的第二个选项修

`_HYBRID` / `_LAST_CTX` / stream_bridge 的 `_recent_ja`/`_last_ctx` 全是单份（代码注释自认
"单客户端假设"），而 8756 绑 0.0.0.0，头显与手机都会直连。

报告给了两条路：按来源隔离，或 inflight 互斥。选了**互斥 + 自愈接管**（改动小、语义明确）：
第一个开始推流的来源独占会话，别的来源收到 503（带档位建议与说明）；最后一个请求过去
30 秒自动释放，避免"客户端崩了/换设备后永久占用"。开关 `server.single_client=false`
可恢复旧行为（明知会串台）。

⚠️ 这是**对头显可见的行为变化**（多设备同时推流时第二台会被拒），需要真机确认头显收到 503
后的表现（重试/提示）。我这边只有单客户端的验证手段，已在报告中标注。

### 验证与牙齿证明汇总（本批）

| 项 | 新代码 | 旧代码（还原后跑） |
|---|---|---|
| F02 缓存成对 | 连跑 5 次全过 | 3/3 失败：`key 与当前 stat 相符、data 落后好几版` |
| F10/F18 计数与时钟 | 通过 | 行为级失败：`读体期间在飞计数应当已经 >0：[0]` |
...
## R85 审查报告第九批修复：F27（.strm 代理 SSRF 对端核对）

### F27 [中] SSRF 防护可被 DNS 重绑定绕过、IPv6 不受检 —— 属实，已修

两处都核实了：`_is_public_http_target()` 用 `AF_INET` **只查 IPv4**（IPv6 的 ULA/回环
完全不受检）；而且校验与 `http.client` **各解析一次** DNS —— 攻击者控制权威 DNS 时可以让
第一次返回公网 IP、第二次返回 127.0.0.1/192.168.x（重绑定 TOCTOU），把 PC 上的 DLNA 服务
当跳板访问内网与回环（8790 宿主 API、8756 字幕、8081/8082）。

修法：
1. 解析改 `AF_UNSPEC`，抽取共用判据 `_is_public_ip()`（IPv4/IPv6 同一套，含 IPv6 的
   `%scope` 处理）；
2. 新增 `_peer_is_public(conn)`：**连接建立后核对真实对端地址**
   （`conn.sock.getpeername()`）——查真实对端才关得掉 TOCTOU 窗口。仍按主机名建连
   （不改成直连 IP），否则 HTTPS 的 SNI 与证书校验会失配；
3. 代理的两条路径都加了这个核对：主路径 + HEAD 被拒后回退 GET 时**新建的那条连接**。
   拿不到对端时 **fail-closed**（返回 False）。

⚠️ 修完 F27 后 **F26 的测试立刻失败**了：那条测试用本机做上游，新的对端核对正确地把它
403 了（守卫在正常工作）。F26 测的是"短读是否关连接"这个独立行为，所以给它也加了
`_peer_is_public` 打桩（并用 `getattr` 兼容旧代码，免得在旧版本上以 AttributeError 结束、
掩盖它真正要证明的东西）。**同一个函数上叠两条修复时，后一条会改变前一条的测试前提。**

验证：`t_dlna_strm_peer_check` —— 策略断言（`AF_UNSPEC`、两处 `_peer_is_public(conn)`）
...
## R87 审查报告第十一批修复：F08 / F09（前端表单与根目录）

### F08 [中] 模型下载完成触发整表重填，静默清掉未保存输入 —— 属实，已修

`loadModels` 的"翻译模型下载完成"分支调用 `loadSubtitleConfig()`，而它把**每个**配置
字段都无检查赋值（`#mtBackend/#mtModel/#mtBase/#mtCloudBase/#mtCloudModel/#mtCloudKey`
/idle 回收…）。用户改到一半（比如正准备填云端 key）时下载一完成，表单被服务端旧值覆盖，
他再点保存就把旧值写回去——以为配好了云端、实际还在 local。`S.subCfgDirty` 只护住了
2.5s 那次补拉。

修法两条：
1. `loadSubtitleConfig({onlyModels:true})`：下载完成只**刷新模型下拉**（用当前下拉值而不是
   配置值作选择，新选项出现的同时保住用户的选择），其余输入一律不碰；
2. 整表重填前先看 `S.subCfgDirty`（与 2.5s 补拉同一判据）；并在**保存成功后**把 dirty
   复位——此前它一旦置真永不复位，加护栏会让后续所有回填永久让路。

### F09 [中] 媒体根目录增删基于客户端缓存全量回写，背靠背操作丢更新 —— 属实，已修

增删都从 `S.settings.dlna_roots`（只靠 1s 轮询刷新）拷一份、改完整体 POST。序列：
删 A → 轮询带回仍含 A 的旧列表 → 添加 B 时把 A 一起 POST 回去（**A 复活**，而 toast 已经
说过"已移除"）。

修法：两条路径都改成**先 `GET /api/settings` 取服务端当前值**再改再提交（服务端权威），
不拿本地缓存当基底。宿主不用改接口。

### 验证（新增 `tests/ui_form_and_roots.js`，无头 Chrome + 注入 fetch 桩）

| 断言 | 新代码 | 旧代码（还原 ui/app.js） |
|---|---|---|
| 下载完成后用户未保存的输入不被覆盖 | ✅ 输入保持原样 | ❌ 被覆写成服务端值 `https://cfg.example/v1` |
| 已删除的根目录不得被"背靠背添加"写回 | ✅ POST `["D:/B"]` | ❌ POST `["C:/A","D:/B"]`（A 复活） |

⚠️ 这条测试**先假通过了两次**，都靠"还原旧代码再跑"才发现桩写错了：
① `renderRoots` 读的是 `/api/state` 的 `dlna.roots`（不是 `settings.dlna_roots`）；
② `loadModels` 读的是 `/api/models/catalog` 的 `r.items`（不是 `/api/models`），而且模型
**必须先 downloading、输入之后再翻成 done** —— 若一开始就是 done，"下载完成"分支在页面
启动那一次就触发了（早于用户输入），断言恒真。教训：**新写的测试必须对着旧代码验一遍
"它真的会失败"**，否则只是好看的绿。

---

## R88 审查报告第十二批修复：F24（MT 逐句路径的熔断门与超时）

### F24 [低] MT 逐句路径无熔断无预算，后端挂起时最坏约 9 分钟/块 —— 属实，已修

报告指 `_fail_streak` / `single_budget` 只存在于批量路径 `work()`，`_translate_mt` 的
`work1` 逐句 `catch Exception` 后继续；`_chat_local` / `_chat_ollama` 的 `_post` 不传
`timeout`，落默认 180s。核实结论：**属实，且比报告写的更严重一层** —— 发运默认配置
（`mode: "auto"` + `mt_system` 非空 + `backend: "local"`）解析出来走的就是 MT 路径，
也就是说 F21 那套"熔断 + 半开恢复"在**默认配置下等于完全不存在**：`_fail_streak`、
`skipped_batches`、`half_open_probes` 三个量在 MT 模式下恒不变化，诊断页永远看不到
"后端已挂"的痕迹，后端一旦"接受连接但不响应"，每句都要白等一个完整 timeout，
跨块无限重复。

### 修法

1. **超时按模式收窄**（`_backend_timeout(section, default)`，从 `local` / `ollama`
   两组读 `timeout_sec`）：MT 逐句 60s，批量 180s（**故意不同**：批量是一次请求翻
   `batch_size` 句，慢机器上 CPU 推理可能真的要分钟级，跟着 MT 一起收窄会把本来能
   翻的批量请求掐死 —— 这不是"判据写两份"，而是两条路径的规模差两个数量级）。
   `timeout_sec <= 0` = 不设上限，与 `openai.timeout_sec`（默认 60）同口径。
   实测数据：本地 7B MT 单句 1.0~1.9s，60s 已是 30 倍余量；旧值 180s 让 10 句块
   （thread_num=4）最坏 `ceil(10/4)×180 ≈ 9 分钟`。
2. **MT 路径补熔断门**：判据与批量模式逐字相同（`streak >= after_fail_batches`、
   冷却期内整块跳过、冷却期外放**一块**探测、跳过不累加、不刷冷却时钟），计量单位
   从"批"换成"块"—— MT 是 1 句 1 批，按批计量会让 1 句失败就熔断整场。
3. **块级失败记账**放在**免费兜底填补之前**取样：兜底能把整块填满译文，但它证明的是
   "免费后端活着"，不是"LLM 后端活着"；按填补后的结果记账的话，配了兜底时 streak
   永远是 0，熔断门等于没装。
4. **不做逐句预算**（报告修法里提到 `single_budget`）：MT 每句**只发一次**请求、没有
   逐句重试，预算防的是"对同一个死后端按句数再撞一遍"的放大效应，MT 侧不存在这个
   放大（真正的防护是 ②的熔断门 + ①的短超时）。写在这里以免下次有人照抄报告又加一遍。

### 验证（`tests/test_pipeline_unit.py` 新增 `t_mt_path_breaker_and_timeout`）

| 断言 | 新代码 | 旧代码（`git stash` 还原 translate_engine.py） |
|---|---|---|
| 整块后端级失败记"一块" | ✅ `streak=1`、`fail_kinds["BackendDown"]=1` | ❌ `streak=0`（熔断门形同不存在） |
| 连续两块失败 → `degraded` | ✅ True | ❌ False |
| 块内有成功句 → 复位 | ✅ `streak=0` | ❌ 保持原值 |
| 整块空译文（内容级）不得推高熔断 | ✅ 复位、`fail_kinds["MTEmpty"]=1` | ❌ 保持原值（F22 口径在 MT 路径失效） |
| 混合失败（BackendDown + ContentBad）以后端级归类 | ✅ `streak=1` | ❌ `streak=0` |
| 熔断期内**一个请求都不发** | ✅ `_chat` 调用数 0、跳过不刷冷却时钟 | ❌ 每句都发（每句白等一个 timeout） |
| 冷却走完放**一块**探测并复位 | ✅ `half_open_probes=1` | ❌ 0 |
| MT 单句超时 60s / 批量 180s / `timeout_sec=0` → None / `=33` → 33 | ✅ | ❌ 180（旧代码不传 timeout，`_post` 默认值） |

牙齿证明：`AssertionError: MT 整块失败没计入熔断（streak=0）⇒ 熔断门形同不存在`。
另外单独探了一次旧代码的 timeout 取值：`OLD mt timeout -> 180`（证明测试第二半也有牙齿，
不会被第一条断言的早失败掩盖）。

⚠️ **我自己第一版实现就被这条测试抓住一次**：v1 只看"块内有没有成功句"，于是
"一整个块都是空译文/漏译"（后端活着、内容不合格）会被算成一次后端级失败，连着几块
就把熔断推满 —— 正是 F22 明令禁止的行为。改法是在 `work1` 里把失败**分类**带回
（`ok` / `empty` / `content` / `down`，`ContentBad` 与空译文归内容级），块级记账时
...
## R92 审查报告第十六批修复：F17（混合切句的 VAD 锁与重复分析）—— 先量后改

### 先测成本，再决定改多少

报告 F17 的三条依据里混着"上限"和"成本"，直接照做会把这一条放大两个数量级。所以先量：
本机 CPU、repo 自带 `audiocpp_cli.exe` + silero（不加载 ASR 模型、不碰 GPU、不惊动正在
运行的实例），单次 `speech_spans` 耗时：

| 音频长度 | 2.0s | 3.5s | 5.0s | 7.0s | 10.0s | `--help`（纯进程启动） |
|---|---|---|---|---|---|---|
| 实测 | 28ms | 32ms | 39ms | 48ms | 62ms | **7ms** |

结论：VAD 是**几十毫秒级**的调用（进程启动只占 7ms，其余是模型加载+推理）。报告里
"超时下限 30s"是**上限**（`timeout = max(30, min(300, dur*2))`），不是每次成本。

### 三条依据逐条核实

| 报告依据 | 核实结果 |
|---|---|
| ① feed() 全程持锁跑 VAD 子进程 | **属实**（锁内 ~30-40ms；partial 的 snapshot 要抢同一把锁 ⇒ 干等） |
| ② 每次未切中的 feed 对全缓冲重跑 VAD | **属实**，但按实测每句只多花约 70ms CPU（3 次调用共 ~110ms，其中约 70ms 是重复分析） |
| ③ hard 切丢弃 feed 期刚算好的 regions，整段重跑 | **不成立**。hard 切是在 RMS 扫描循环里定的（`if i > max_n`），而 VAD 代码块有 `if not cut_n` 前置条件 ⇒ hard 切那一次 feed **根本没跑 VAD**，没有 regions 可丢。探针实测确认：`feed0 vad_calls=[2.0] / feed1 [2.0,4.0] / feed2 hard cut ⇒ 调用数不变、last_cut_regions=None`。**每次 hard 切全程恰好 1 次 VAD**（server_app 为对齐跑的那次，长度=span），不存在"重跑近乎相同音频" |

### 改了什么

**partial 不再被锁内的 VAD 卡住**：`feed()` 在锁内发布一份 `(pcm 拷贝, start_ms)`
（几百 KB memcpy，微秒级），`snapshot()` 改成**无锁读**这份发布物（返回前再拷一份给
调用方，避免 ASR 侧动到发布物）。发布物是不可变引用（缓冲只做整体替换：concatenate /
切片），读侧不存在撕裂。

**顺带加固**：`last_cut_regions` 现在统一走 `_clip_regions(vad_regions, cut_n)` ——
把语音区截到切点之前，保证"返回的区一定落在 span 内"这个不变量显式成立；`silent`
分支顺手把 `last_cut_regions` 清成 None（避免将来有调用方读到上一次的陈旧值）。
⚠ 这一条在当前唯一可达路径（reason=="vad"）下与旧行为**完全等价**，是加固不是修复，
不当成 F17 的修复成果记账。

### 有意没做的两件（附理由）

1. **"feed 间只评新增尾部"（增量 VAD）**：它改变喂给 silero 的**窗口** ⇒ 切点、
   对齐组、时间戳都可能随之变化，而这台机器上没法用真实音频+模型验证（用户正在用，
   R76 的教训）。收益按实测只有约 70ms/句 CPU，风险与收益不对等。
   *若日后真要做，正确入口是常驻 VAD 进程*（省掉每句 3 次进程启动 ≈ 21ms），
   而不是改窗口 —— 而按上面的实测，连这个也不值得。
2. **"VAD 移到锁外"（并发）**：partial 这条最用户可见的路径已由上面的改动解除；
   剩余暴露只有"并发 feed 撞上 40ms 的 VAD"（单客户端独占 F11 下罕见，只有同一台
   头显流水线发包时才会遇到），以及"VAD CLI 挂死 30s 上限"这种罕见失效。要在时序
   状态机里为此引入 generation 校验 + 重试路径，风险不对等，本轮不做（记录在案）。

### 验证（`tests/test_pipeline_unit.py` 新增 1 项检查）

| 断言 | 新代码 | 旧代码 |
|---|---|---|
| 锁内 VAD 进行中，`snapshot()` 仍能立刻返回上一次发布的快照 | ✅ <1ms | ❌ **等了 10005ms**（被锁挡住，直到慢 VAD 超时） |
| hard 切：全程恰好 1 次 VAD（2.0s / 4.0s 两次未切中 + 对齐 5.2s），`last_cut_regions is None` | ✅ | ✅（**特征锁**，两版都过 —— 正说明报告③不成立） |
| vad 切：feed 期 regions 被复用，不再为对齐重跑（调用序列 `[2.0, 3.5]`） | ✅ | ✅（特征锁） |
...
## R95（2026-09-25 凌晨）：ASR 切 GPU（D 盘实机）+ 8791 卡死 bug 首次实录
...
## R98（2026-09-25）：工作流组合矩阵评测 + 评分机制大修（用户点名"不要照用现有评分机制"）

**做法**：ZCode 动态工作流（7 阶段：准备+审计并行 → 独立复核 → 改进评分器 → 4 组合矩阵 → 翻译盲评 → 调优 → 恢复），隔离服务 :8759/:18081/:18082 不碰生产，SIVR-002 90-690s + 180s 实时延迟推流。

**审计结论：现有评分机制确有大洞（12 条 confirmed，4 条 high）**：
1. **延迟锚点错**：measure_emission_lag.py:92 实算「块末端−段start」而文档声称「−段end」→ R63.4 起的延迟数被虚增整句时长（实测旧口径 p50=2540ms，新口径 1378ms）。
2. **延迟没量到最终中文译文**：seen 以日文文本为键、空译文段照样算"已出字"、响应自带 mt_ms/total_ms 被丢弃 → 旧数字只是协议下限。
3. **覆盖率纯时间重叠零文本校验**：乱码/复读都算"识别到"（R60 的"换 ASR 召回不动"正是被这个口径遮蔽——时序配对由切句器决定，文本差异完全不可见）。
4. **幻觉零罚则 + 服务端复读置空缓解被记成缺陷**；另有 LCS 只奖漏不罚错、无全半角折叠、时序无 end 误差/p90/max 用 abs(dts[-1]) 的 bug、长度比空译文稀释、partial 静默当漏识、无机器可读分数无回归门槛、延迟与质量测不同视频等。

**新评分器 tests/score_combo.py**（文本佐证命中、幻觉率、start/end 双误差 p50/p90、长度比分位数+自校准带、翻译出字延迟=句末→最终译文首现块末端、缺译/neverEmitted 单列、sanity 自测）。**新驱动 tests/run_combo_eval.py**（预置配置换模型组合、端口隔离、code_sig+asr_model 双校验；已知 bug 已修：overlap-sec 曾被 int 截断）。

**矩阵结果（0.6B/1.7B × Sakura-7B/1.5B，全 GPU）**：
- 召回：0.6B=0.65/0.633，**1.7B=0.733**；精度：0.6B=0.585，**1.7B=0.646**（时间重叠口径下四组完全相同——旧口径看不见的差距）
- 幻觉率：0.6B=16.4%，**1.7B=26.4%**（もっと/うん 类短叹词），精度仍 1.7B 胜
- 时序（p50 252ms/p90 946ms）与**翻译出字延迟（p50 1.378s/p90 2.202s）四组完全相同**——由切句器与块网格决定，与 ASR/翻译模型无关
- 翻译盲评（3 评审独立、金标准日文为参考、匿名）：0.6B 组 5.81/5.71，**1.7B 组 7.29/7.26**——ASR 文本质量主导翻译质量，同 ASR 下 7B≈1.5B
- 墙钟：0.6B 组 ~100-106s；**1.7B+7B=351s（显存爆压，8GB 卡装不下 1.7B+7B）**；**1.7B+1.5B=111s 无代价**

**结论与建议**：**1.7B+1.5B 是有代价无质量损失的最优组合**（质量全面 ↑、延迟/速度不变、显存可容），R60"维持 0.6B"的结论被新口径推翻（旧口径量不出差距）；但换产前须全片复测+真实并发显存验证。调优实验：beam_size 不支持（audio.cpp qwen3_asr 无该选项，实测+上游文档双证）；chunk 6s 吞吐×2.2 但延迟崩坏（p50 28s）否决；overlap 0.5 延迟反升否决——**现役 3s/1s 就是延迟最优档**。
