...
**结论（两轮定案）**：旧批量翻译时代 10s 最优（3s 小批噪音多）；**换 Sakura 逐句 MT 后重评，
3s/1s 反超**（召回 90.3% 最高、时序 −102ms 最准、首条 ~4-5s）——批量质量理由消失，**定 3s/1s**，
头显 `STREAM_CHUNK_SEC=3 / STREAM_OVERLAP_SEC=1`（v1.6.14 起）。30s 滞后 33s+ 且覆盖最差，排除。
...
### 9.1 重出的质量基线（R42 修复后整片实测，取代 §6 旧数字）

整片 1254s，offline 传输，跑的是**仓库代码 + 自起隔离实例 :8759**（不碰生产），
签名 `code_sig=641421ea1571`：

| 指标 | 旧值（错口径） | **3s/1s（生产档）** | **25s/2s** |
|---|---|---|---|
| 语音覆盖召回 | 90.3% | **98.4%** | 96.7% |
| 漏识 | 9.7% | **1.6%** | 3.3% |
| 时序中位（可信） | −102ms | **−90ms** | −160ms |
| ±500ms | 68.8% | 68.6% | **76.2%** |
| 覆盖率（**上界**） | 0.478 | 0.488 | 0.477 |
| 译文为空 | 2.1% | 0.6% | 0.6% |
| wall | — | 501s | **167s** |

**三条读法**：① 召回提升**主要来自口径修正**（旧口径下 20 条短句必然被判假漏识），
不能当作识别质量跳变；② **覆盖率没变**，说明 R42 的判据修复没有体现在这个指标上，
"修复让字幕变好"目前只有判据层 A/B 证据；③ **3s 档覆盖率更虚高**（20% 的拼接文字
落在人工句范围外，25s 档仅 4%），两档 0.488 vs 0.477 的差不可直接比较；
25s 档快 3 倍且 ±500ms 更好，**档位权衡值得按新数字重看**。
